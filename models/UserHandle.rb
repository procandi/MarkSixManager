...
class UserHandle
  ...
  def InsertPatient(chartno,chartname,citizenid,birthday,sex,address,phone,mobile)
    @chartno=chartno
    @chartname=chartname
    @citizenid=citizenid
    @birthday=birthday
    ...
    sql="insert into "
    sql+="cris_patient_info( "
    sql+="chartno,name,citizenid,birthday,sex,address,phone,mobile "
    sql+=") values( "
    sql+="'#{@chartno}', "
    if @db::dbtype=="MSSQL"
      sql+="N'#{@chartname}', "
  ...
  def UpdatePatient(chartno,chartname,citizenid,birthday,sex,address,phone,mobile)
    @chartno=chartno
    @chartname=chartname
    @citizenid=citizenid
    @birthday=birthday
  ...
  #get scopy list
  def GetScopyList(begindate,enddate,aetitle)
    sql="select "
    sql+="distinct a.uni_key,a.createdate,a.createby,b.scope_list "
    sql+="from "
    sql+="cris_images_reference a,cris_colon_data b "
    sql+="where "
    sql+="a.uni_key=b.uni_key and a.createby is not null and ("
    sql+="a.createdate>='#{begindate}' and " if begindate!=''
    sql+="a.createdate<='#{enddate}' and " if enddate!=''
    sql+="a.createby like '%#{aetitle}%' and " if aetitle!=''
    sql+="1=1) "
    @logger.info(sql)

    result=''
    @db.dbh.select_all(sql){|rec|
      result+="#{rec[0]},#{rec[1]},#{rec[2]},#{rec[3].to_s().gsub(/,/,'@')}\n"
    }
    result=result[0..result.length-2] if result!=''
    
    return result
  end
  
  
  #get scopy use counting
  def GetScopyUseCounting(begindate,enddate,scopy)
    sql="select "
    sql+="distinct a.uni_key,b.scope_list "
    sql+="from "
    sql+="cris_images_reference a,cris_colon_data b "
    sql+="where "
    sql+="a.uni_key=b.uni_key and a.createby is not null and ("
    sql+="a.createdate>='#{begindate}' and " if begindate!=''
    sql+="a.createdate<='#{enddate}' and " if enddate!=''
    sql+="b.scope_list like '%#{scopy}%' and " if scopy!=''
    sql+="1=1) "
    @logger.info(sql)
    ...
    result=result[0..result.length-1] if result!=''
